...
export default async function DashboardPage({
  ...
  if (phase12) {
    if (gmail?.status === "needs_reconnect") {
      warnings.push("Gmail token expired — reconnect Gmail to resume importing emails");
    ...
    <div className="space-y-6">
        ...
        <Card title="Status">
          {phase12Ok ? (
            ...
            <p className="text-sm text-emerald-700">
              Gmail connected — new Airbnb confirmation emails will appear under Bookings
              automatically.
            </p>
          ) : warnings.length === 0 ? (
        ...
        <Card title="Recent bookings">
          {(recentBookings ?? []).length === 0 ? (
            <p className="text-sm text-zinc-600">
              No bookings yet. After Gmail is connected, confirmation emails appear here with dates
              and financials.
            </p>
          ) : (
