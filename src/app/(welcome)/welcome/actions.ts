...
export async function submitWhatsAppNumber(formData: FormData) {
  ...
  // Outbound outside a 24h session must use an approved template (not plain text).
  const template = getTemplateRef("host_welcome", "en");
  const result = await getWhatsAppClient().sendTemplate(phone, template, {});
