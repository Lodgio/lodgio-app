...
export async function runMatchingForHost(hostId: string) {
  ...
  // Repair: form already matched, but Gmail re-ingest previously reset booking status.
  ...
  for (const booking of stuck ?? []) {
    ...
    await supabase.from("bookings").update({ status: "matched" }).eq("id", booking.id);
...
export async function matchSubmission(submissionId: string) {
  ...
  const claimedId = submission.claimed_airbnb_booking_id.trim();

  const { data: exactBooking } = await supabase
    .from("bookings")
    .select("*")
    .eq("host_id", submission.host_id)
    .eq("airbnb_booking_id", claimedId)
    .maybeSingle();

  let booking = exactBooking;

  // Case-insensitive fallback if guest typed code with different casing
  if (!booking) {
    ...
    booking =
      (candidatesByCode ?? []).find(
        (c) => c.airbnb_booking_id.trim().toLowerCase() === claimedId.toLowerCase()
      ) ?? null;
  }
