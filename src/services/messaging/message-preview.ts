...
export async function buildBookingMessages(
  ...
  const sharedValues = {
    ...
    guest_phone: guest?.whatsapp_number ?? null,
    property_name: property?.name ?? null,
    location_url: property?.location_url ?? null,
    caretaker_name: caretaker?.name ?? null,
    caretaker_phone: caretaker?.phone ?? null,
    check_in: formatDate(booking.check_in),
    check_out: formatDate(booking.check_out),
    check_in_time: booking.check_in_time ?? property?.check_in_time ?? null,
    // Weather is not implemented yet — omit the whole section.
  ...
  const messages: BookingMessagePreview[] = [
    {
      ...
      body: composeMessageBody("guest_welcome", sharedValues),
    },
  ];
  ...
  if (caretaker) {
    messages.push({
      ...
      body: composeMessageBody("caretaker_notify", sharedValues),
    });
  }
